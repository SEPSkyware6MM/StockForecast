...
\title{Stock forecasting methods and their usage}
...
Mannheim, Germany\\
Email: marc.misoch@gmail.com}
\and
\IEEEauthorblockN{David Marquant}
\IEEEauthorblockA{Institute of Computer Science\\University of Applied Science\\
Mannheim, Germany\\
Email: david.ma@live.com}
}
...
\begin{IEEEkeywords}
stock, forecast, paper, algorithms.
\end{IEEEkeywords}
...
%Stocks are also influenced by numerous factors and these factors also have relationships between each other. This leads us to the thought, that a good stock prediction has to consider more than just old stock prices. 
\\
...
\\
In this paper we are going to compare some algorithms and their functionality. We also like to share some of our experimental results we had with the algorithms. We only used algorithms that do not take global news in account because we wanted to see, how good the algorithms were in an isolated environment analysing past prices only. We decided to do it that way, because the algorithms themselves should work properly and examining news would be an easy addition to these working algorithms. The problem with stock forecasting is, that the market is global. 


% ABSCHNITTS FUTURE WORK This newscrawler linkage would be a good subject for future research.
...
% beschreiben wie der algorithmus von Herr Fischer funktioniert und die 2-3 die wir noch implementiert haben

\section{Algorithms}

We implemented three different algorithms to predict stock prices and maximise the outcome. All three of them only consider one stock at a time. In this section we will describe the algorithms we used

\subsection{Algorithm using the latest trend}

TODO: algorithmus beschreiben mit dem neuesten Trend der beispielweiße die nächsten 5 tage vorraussagen kann.
...
\subsection{Algorithm using neuronal network}

This algorithm is based on neuronal networks. 

TODO: den algorithmus von Herr Fischer beschreiben.
